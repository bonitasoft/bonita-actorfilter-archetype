...
The project **actorfilter-java-test** is a Bonita Actor Filter for **Bonita {short-bonita-version}**  **Community** version written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/actorfilter-java-test.git
cd actorfilter-java-test/
mvn package
----

The build should produce a zip archive under the `target/` folder named `actorfilter-java-test-1.0.0-all.zip`
...
Take the built artifact `actorfilter-java-test-1.0.0-all.zip` and import it to your Bonita Studio.
